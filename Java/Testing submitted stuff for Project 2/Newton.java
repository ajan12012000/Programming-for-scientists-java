...
class Newton {
    ...
    public void iterate(Complex z0) {
    	...
    	err = 0;
    	numIterations = 0;
    	if (getFp().evaluate(root).abs() == 0) {
    		err = -1;
    		return;
    	}
    	Complex root1 = z0.add(getF().evaluate(z0).divide(getFp().evaluate(z0)).minus());
    	
    	while (root1.add(root.minus()).abs() > TOL ) {
//    		System.out.println("z = " + getRoot());
        	numIterations += 1;
        	root1 = root;
        	root = root.add(getF().evaluate(root).divide(getFp().evaluate(root)).minus());;
//        	h = getF().evaluate(getRoot()).divide(getFp().evaluate(getRoot())) ;
        	
        	if (getNumIterations() == MAXITER) {
        		err = -2;
        		break;
        	}
        	if (getFp().evaluate(root).abs() == 0) {
